...
- Android home screen widgets are implemented using `react-native-widget-extension`
- Swift widget source files live in `widgets/` at the repo root
- Shared app data is written by the app and read by widgets via App Group / shared storage
- Widget refresh cadence: 30 minutes via `AppWidgetProvider` on Android
- Register the widget config plugin in `app.json` under `plugins`
- Run `bun run native:sync` after adding or modifying widget files
